...
/*
[Script]
http-request ^https:\/\/api\.pingmeapp\.net\/app\/queryBalanceAndBonus script-path=PingMe_Loon.js, tag=PingMe抓参, requires-body=false, timeout=60
cron "30 8,20 * * *" script-path=PingMe_Loon.js, tag=PingMe签到
...
if (isRequest) {
  ...
  if (!raw) {
    notifyDone('⚠️ 未抓到参数', '先打开 PingMe 触发一次 queryBalanceAndBonus');
    done();
  } else {
    let capture;
    try {
          ...
          setTimeout(() => {
            ...
            fetchApi('videoBonus', capture, headers).then(res => {
              try {
                ...
                if (d.retcode === 0) {
                  msgs.push(`🎬 视频${i}：+${d.result && d.result.bonus ? d.result.bonus : '?'} Coins`);
                  resolve(next());
                } else {
    ...
    fetchApi('queryBalanceAndBonus', capture, headers)
      .then(res => {
        try {
          const d = JSON.parse(res.body);
      ...
      .then(res => {
        try {
          const d = JSON.parse(res.body);
          if (d.retcode === 0) {
            msgs.push(`✅ 签到：${(d.result && d.result.bonusHint ? d.result.bonusHint : d.retmsg || '').replace(/\n/g, ' ')}`);
          } else {
